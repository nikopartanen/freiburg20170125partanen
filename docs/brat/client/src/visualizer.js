...
var Visualizer = (function ($, window, undefined) {
    ...
    Span.prototype.initContainers = function (offsets) {
        ...
        // for box, cross, etc. that are span-global
        this.fragments =[];
        this.normalizations =[];
    };
    ...
    var setCollectionDefaults = function (collectionData) {
        ...
        'relation_types',
        'unconfigured_types',], function (attrNo, attr) {
    ...
    var Visualizer = function (dispatcher, svgId, webFontURLs) {
        ...
        // effect is inverse of "margin" for some reason
        var highlightRounding = {
            x: 3, y: 3
        };
        // rx, ry for highlight boxes
        var spaceWidths = {
            ' ': 4,
            '\u00a0': 4,
            '\u200b': 0,
        ...
        var setData = function (_sourceData) {
            ...
            $.each(sourceData.attributes, function (attrNo, attr) {
                ...
                var attrText = attrType ? (attrType.bool ? attrType.name: (attrType.name + ': ' + valText)): (attr[3] == true ? attr[1]: attr[1] + ': ' + attr[3]);
            ...
            $.each(sourceData.comments, function (commentNo, comment) {
                ...
                if (comment[0] instanceof Array && comment[0][0] == 'sent') {
                    ...
                    var commentEntities =
                    trigger ? trigger[1] // trigger: [span, ...]: id in data.spans ?[data.spans[id]] // span: [span]: id in data.eventDescs ?[data.eventDescs[id]] // arc: [eventDesc]:[];
            ...
            markedText =[];
            setMarked('edited');
            // set by editing process
            setMarked('focus');
            // set by URL
            setMarked('matchfocus');
            // set by search process, focused match
            setMarked('match');
            // set by search process, other (non-focused) match
        ...
        var getTextAndSpanTextMeasurements = function () {
            ...
            var fragmentSizes = getTextMeasurements(fragmentTexts, {
                'class': 'span'
            });
        ...
        var addArcTextMeasurements = function (sizes) {
            ...
            var arcSizes = getTextMeasurements(arcTexts, {
                'class': 'arcs'
            });
            sizes.arcs = arcSizes;
        };
        ...
        var renderDataReal = function (sourceData) {
            ...
            var backgroundGroup = svg.group({
                'class': 'background'
            });
            var glowGroup = svg.group({
                'class': 'glow'
            });
            highlightGroup = svg.group({
                'class': 'highlight'
            });
            var textGroup = svg.group({
                'class': 'text'
            });
            ...
            $.each(data.spanDrawOrderPermutation, function (spanIdNo, spanId) {
                ...
                floorNo++) {
                    ...
                    for (var i = i1; i <= i2; i++) {
                        ...
                        // XXX maybe add fragment; probably unnecessary
                    }
                }
            ...
            $.each(data.chunks, function (chunkNo, chunk) {
                ...
                $.each(chunk.fragments, function (fragmentNo, fragment) {
                    ...
                    fragment.rectBox = {
                        x: bx, y: by - span.floor, width: bw, height: bh
                    };
                    ...
                    if (fragment.drawCurly) {
                        ...
                        svg.path(fragment.group, svg.createPath().move(fragment.curly.from, bottom + Configuration.visual.curlyHeight).curveC(fragment.curly.from, bottom,
                        x, bottom + Configuration.visual.curlyHeight,
                        x, bottom).curveC(x, bottom + Configuration.visual.curlyHeight,
            ...
            $.each(data.arcs, function (arcNo, arc) {
                ...
                for (var rowIndex = leftRow; rowIndex <= rightRow; rowIndex++) {
                    ...
                    if (row.chunks.length) {
                        ...
                        (leftToRight ? 'none': myArrowHead || 'triangle,5')) + ',' + color;
            ...
            $.each(rows, function (rowId, row) {
                ...
                var rowBoxHeight = Math.max(row.maxArcHeight + 5, row.maxSpanHeight + 1.5);
                // XXX TODO HACK: why 5, 1.5?
                ...
                if (row.sentence) {
                    var sentence_hash = new URLHash(coll, doc, {
            ...
            $.each(textMarkedRows, function (textRowNo, textRowDesc) {
                ...
                // TODO: put into css file, as default - turn into class);
        ...
        var collectionLoaded = function (response) {
            if (! response.exception) {
                ...
                var arcBundle = (response.visual_options || {
                }).arc_bundle || 'none';
                var textDirection = (response.visual_options || {
                }).text_direction || 'ltr';
